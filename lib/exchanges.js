...
async function fetchPrices(exchange, symbols) {
...
async function fetchBinanceEarnBalances(exchange) {
  const earnAssets = [];

  try {
    // Binance Simple Earn Flexible positions
    const flexibleResponse = await exchange.sapiGetSimpleEarnFlexiblePosition({});
    ...
    try {
      const savingsResponse = await exchange.sapiGetLendingUnionAccount({});
  ...
  try {
    const lockedResponse = await exchange.sapiGetSimpleEarnLockedPosition({});
...
async function fetchKucoinEarnBalances(exchange) {
  const earnAssets = [];

  try {
    // KuCoin Earn (savings) positions
    const response = await exchange.privateGetEarnOrders({
      status: 'HOLDING',
    });
    ...
    try {
      const savingsResponse = await exchange.privateGetSavingsHolding({});
...
async function fetchGateEarnBalances(exchange) {
  const earnAssets = [];
  ...
  try {
    ...
    // Try alternative endpoint
    try {
      const response = await exchange.privateEarnGetUniLendRecords({});
      if (Array.isArray(response)) {
        for (const item of response) {
          const amount = parseFloat(item.amount || '0');
          if (amount > 0 && item.currency) {
...
async function fetchEarnBalances(exchange, exchangeName) {
  switch (exchangeName) {
    case 'binance':
